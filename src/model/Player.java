...
public class Player extends Thread implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5168720926367665813L;
	...
	private static final int rotationSpeed = 3; // the speed at which each tank
												// can turn
	private static final double forwardSpeed = 1; // the speed at which each
													// tank can move forwards
	private static final double reverseSpeed = -.65; // the speed at which each
														// tank can move
														// backwards
	...
	private void move(double speed) throws IOException {
		...
		if (cornerCrash(nextPoint, GameEngine.tankWidth)) {// if the tank would
															// crash into a
															// corner...
			nextPoint = new Point(this.coordinates.getxCoord()// try moving it
																// horizontally
			...
			if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth)
				...
				nextPoint = new Point(this.coordinates.getxCoord(), // if moving
																	// it
																	// horizontally
																	// would
																	// cause a
				...
				if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth)
						|| wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
					nextPoint = new Point(this.coordinates.getxCoord(), // if
																		// moving
																		// it
																		// vertically
																		// would
																		...
																		// move
																		// it
							this.coordinates.getyCoord());// TODO stop it going
															// through T walls
				}
			}
		} else {
			...
			if (wallCrashVertical(nextPoint, GameEngine.tankWidth)) { // if the
																		// tank
																		// would
																		// go
																		// over
				...
				nextPoint = new Point(this.coordinates.getxCoord(), // just move
																	// it
																	// vertically
						this.coordinates.getyCoord() - (speed * Math.sin(Math.toRadians(90 - this.direction))));
			}
			if (wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {// if the
																		// tank
																		// would
																		// go
																		// over
				...
				nextPoint = new Point(
						this.coordinates.getxCoord()// just move it horizontally
	...
	private boolean wallCrashVertical(Point p, int w) {
		// when given a point p, returns whether or not the circular object of
		// width w at that point is in/over a wall
		w = w / 2;
		int xWall = 0;
	...
	private boolean cornerCrash(Point p, int w) {
		// when given a point p, returns whether or not the circular object of
		// width w at that point is in/over the nearest corner
		w = w / 2;
		...
		// work out if there's a wall in the nearest corner}
		if (!isWallInCorner) {
			return false;// if there's no wall there, there's nothing to crash
							// into
		}
	...
	public void shoot() {
		if (this.numberOfBulletsFired < 5) {
			...
			if (wallCrashVertical(bulletStart, GameEngine.bulletWidth)
					...
					|| cornerCrash(bulletStart, GameEngine.bulletWidth)) { // if
																			// the
																			// bullet
																			// would
																			// start
				...
				engine.bulletList.add(new Bullet(playerNumber, bulletStart, direction, engine));
				// create a new bullet instance, travelling in the direction
				// currently faced, with coordinates just in front of where the
				// tank
	...
	public double currentXSquare() {
		// return the x value of the grid square the centre of the tank is
		// currently in
		for (int i = 0; i < 7; i++) {
	...
	public double currentYSquare() {
		// return the y value of the grid square the centre of the tank is
		// currently in
		for (int i = 0; i < 7; i++) {
	...
	public Player(int playerNo, double x, double y, GameEngine e, Socket client) throws IOException {
	...
	@Override
	public void run() {
		boolean[] instructionsArray = null;
		while (true) {
			try {
				writer.writeObject(engine);
				writer.flush();
				if (playerNumber == 1) {
					instructionsArray = (boolean[]) reader.readObject();
					...
					for (int i = 0; i < engine.bulletList.size(); i++) {
						engine.bulletList.get(i).moveBullet();// move it
						boolean removed = engine.bulletList.get(i).reduceTimer();// count
																					// down
																					// it's
																					// timer
						if (!removed) {// if the bullet hasn't run out...
							engine.bulletList.get(i).tankCollision();// check
																		// for a
																		// collision
						}
					}
